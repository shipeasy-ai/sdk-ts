// Shared types for the headless devtools core (`@shipeasy/devtools-core`).
// Framework-agnostic: no DOM, no React, no React Native imports. The RN overlay
// (`@shipeasy/react-native-devtools`) and the browser overlay
// (`@shipeasy/browser-devtools`) both consume these.
//
...
// The overlays import every shared symbol through this module. The bridge,
// capability payload, override-cookie format, i18n markers and see() builders
// are the SDK↔overlay contract and live in the SDK; re-export them here so
// overlay code has a single import surface (as it did pre-split).
export * from "@shipeasy/sdk/devtools-contract";
